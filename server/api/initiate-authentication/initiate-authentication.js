...
apiRouter.get('/initiate-authentication', async (req, res) => {
  try {
    ...
    // Check if default documents are being served
    if (isDefaultDocumentEnabled(req)) {
      res.json({ redirectUrl: twitterAuthUrl }); // Send Twitter authorization URL in the response body
    } else {
...
// Function to check if default documents are being served
